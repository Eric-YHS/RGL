...
首次分配后，将材料和两道操纵检验题的选项排列保存在当前浏览器的 `localStorage`。同一 `pid`（也接受 `participant_id`）再次进入时优先使用该记录，链接中的新 `treatment` 不会覆盖原材料。不同编号分别存储；无编号时沿用该浏览器的匿名记录。

首次未传 `treatment` 时，有 `pid` 则按其确定性哈希分组，无 `pid` 则随机分配。每人只在首次打开时随机生成选项排列，以后重复进入不再改变。这只保存分配和选项顺序，不恢复答题进度。更换浏览器、域名或清除网站存储后不能保证恢复原记录；禁用存储时仅保证本次页面内顺序稳定。
